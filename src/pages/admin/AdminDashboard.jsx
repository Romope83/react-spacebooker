  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={loading} className="w-full bg-indigo-600 text-white px-4 py-2.5 font-semibold rounded-lg hover:bg-indigo-700 disabled:bg-indigo-400 disabled:cursor-not-allowed transition-colors">
          ...
          <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto pr-2 divide-y divide-slate-100">
              {spaces.length > 0 ? spaces.map((space, index) => (
                <div key={space.id} className={`pt-${index === 0 ? '0' : '4'} pb-2 flex justify-between items-center`}>
                  <div>
                  ...
                  <button onClick={() => handleDeleteSpace(space.id)} className="text-slate-400 hover:text-rose-500 p-2 rounded-full transition-colors"><TrashIcon /></button>
                </div>
              )) : (
                <p className="text-slate-500">Nenhum espaço cadastrado ainda.</p>
              )}
            </div>
          </div>
        </div>
